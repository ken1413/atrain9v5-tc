...
把遊戲的日文介面即時顯示為台灣繁體中文。共 5,834 條對照，
**涵蓋率保守估計約九成**——選單、報表、建設項目、車輛與地圖說明大致都翻了，
少數角落可能還是日文。

**遊戲語系請設為「日本語」**，英文模式下不會生效。
...
>
> 也特地把字型檔還原成原廠狀態測過一次「乾淨安裝」：程式從原廠的
> `un02.bin`（360 bytes）補出 1,906 個中文字元到暫存區，產生的字元集合
> 與長期使用中的版本完全相同，遊戲目錄的檔案測完仍與原廠位元組相同。
...
> ## ⚠ 先決條件：遊戲語系要設成「日本語」
>
> 本工具的原理是**把畫面上的日文字串就地換成中文**，
> 所以遊戲本身必須跑在日文模式。設成英文的話，記憶體裡是英文字串，
> 一條都比對不到，畫面會維持全英文。
>
> 設定方式（安裝前先做，遊戲內就能改）：
>
> **Option（オプション）→ 言語オプション → 日本語を選択**
>
> 選好後照遊戲指示重開，確認介面是日文，再繼續下面的安裝步驟。
>
> 遊戲預設就是日文，沒動過的話不用理會這一段。
...
| 有 log 但**畫面全是英文** | 遊戲語系設成英文了。改回日本語（見上方「先決條件」）|

## 已知限制

- **只支援日文模式的遊戲**，英文模式下完全不生效。
